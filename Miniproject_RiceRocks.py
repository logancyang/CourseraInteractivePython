...
def group_collide(group, other_obj):
    ...
    for sprite in set(group):
        if sprite.collide(other_obj):
            ...
            collide = True
    if collide == True:
        new_explosion = Sprite(other_obj.get_position(), [0, 0], 0, 0, explosion_image, explosion_info)
    ...
    def draw(self, canvas):
        if self.animated:
            ...
            canvas.draw_image(explosion_image, 
                    [explosion_info.get_center()[0] + explosion_index[0] * explosion_info.get_size()[0], 
                     explosion_info.get_center()[1] + explosion_index[1] * explosion_info.get_size()[1]], 
                     explosion_info.get_size(), self.pos, self.image_size)
        else:
            canvas.draw_image(self.image, self.image_center, self.image_size,
                          self.pos, self.image_size, self.angle)
...
def draw(canvas):
    ...
    # check if ship hits rock_group
    if group_collide(rock_group, my_ship):
        lives -= 1

    # check if missile hits rock
...
def rock_spawner():
    global rock_group
    rock_counter = len(rock_group)
    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
    rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
    rock_avel = random.random() * .2 - .1
    if rock_counter < 12 and started:
        new_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        if dist(rock_pos, my_ship.get_position()) > my_ship.get_radius() * 3:
